...
To use this library, copy the library portion into your script.
The start and end of the library are marked with comments.
"""
...
def _gs_namespace():
    ...
    class _Clickable:
        ...
        def on_click(self, handler: Any, args: "Optional[List[_Block]]" = None):
            """
    ...
    class _GumpBuilder(_Clickable):
        ...
        class Scope:
            def __init__(self, builder: "_GumpBuilder", block: _Container):
        ...
        def __init__(self, movable: bool = True, closable: bool = True, disposable: bool = True):
...
################################################################################
# Example 1: Custom Gump Style
################################################################################

"""
This example demonstrates how to create a custom gump style by subclassing
GumpBuilder and adding custom block types. It then constructs a sample gump
using the custom builder.

This is not part of the library itself, but serves as an example of how to
extend and use the GumpBuilder class.
"""
...
class CraftingGumpBuilder(GumpBuilder):
    ...
    class _ProgressBar(GumpBuilder.Assets.Container):
        """This is an example of a custom block type."""
    ...
    def MainFrame(self, spacing: int = 5):
        """
        A shorthand for adding the main frame of the crafting gump style.
        """
        return self.Column(padding=10, background="frame:5054", spacing=spacing)
    ...
    def LabeledButton(self, label: str, up: int = 4005, down: int = 4007, hue: int = 1152, width: Optional[int] = None):
        """
        ...
        with self.Row() as row:
            btn = self.Button(up=up, down=down)
            self.Spacer(5)
            self.Text(label, hue=hue, width=width)
        return btn

    def ItemDisplayButton(self, item: "Union[Item, int]", selected: bool = False):
        """
        ...
        if isinstance(item, int):
            ...
            if item_new is None:
                return self.Checkbox(up=2328, down=2329, width=80, height=60)  # Empty button
            item = item_new
        btn = self.Checkbox(up=2328, down=2329, width=80, height=60, itemproperty=item.Serial)
...
################################################################################
# Example 2: Building and Launching a Gump
################################################################################
...
if __name__ == "__main__":
    ...
    with gb.MainFrame(spacing=5):
        with gb.ShadedColumn(halign="center"):
            gb.Html("Welcome to the Crafting Gump!", width=300, height=22, color="#FFFFFF", centered=True)
            btn_inventory = gb.ItemDisplayButton(Player.Backpack)
            prog_bar = gb.ProgressBar(width=200, height=22, progress=0.2)
        with gb.Row(spacing=5):
            with gb.ShadedColumn(spacing=5):
                with gb.Row(spacing=5):
                    gb.GumpArt(graphics=2373, width=44, height=44)
                    gb.Text("Hello, World!", hue=1152)
                with gb.Row():
                    gb.Text("Name:", hue=87, width=40)
                    name = gb.TextEntry("(Your name here)", width=150, hue=1152, max_length=30, tooltip="Enter your name here")
                with gb.Row(spacing=5):
                    gb.Checkbox(checked=True, tooltip="Check this to enable nothing.")
                    gb.Text("I do nothing", hue=67)
            with gb.ShadedColumn():
                with gb.Row():
                    gb.LabeledButton("Click me!").on_click(lambda: Misc.SendMessage("You clicked me!", 88))
                with gb.Row():
                    btn_submit = gb.LabeledButton("Submit Name")
                with gb.Row():
                    gb.LabeledButton("Just close me", up=4017, down=4019).on_click(True)  # Close the gump

    # Triggered when the gump is closed without pressing any button
    gb.on_exit(lambda: Misc.SendMessage("You can close the gump when the progress bar is full!", 88))

    # Triggered when the the submit button is pressed
    def _get_name(text_entry):
        # This accepts the text entry block as argument
        Misc.SendMessage(f"You entered: {text_entry.text}", 88)
        return text_entry.text

    btn_submit.on_click(_get_name, [name])
    
    # Triggered when the item button is pressed
    def _toggle_item(button):
        if button.checked:
            Misc.SendMessage("I'm checked!", 68)
        else:
            Misc.SendMessage("I'm unchecked!", 33)
    
    btn_inventory.on_click(_toggle_item, [btn_inventory])

    # Loop until the gump has response or progress bar is full
    while True:
        # Launch the gump and wait for response
        block, result = gb.launch()
        if result is not None:
            break
        # Simulate progress
        if prog_bar.progress > 0.9999:
            Misc.SendMessage("Full progress!", 88)
            break
        prog_bar.progress += 0.1


__export__ = ["GumpBuilder", "CraftingGumpBuilder"]
